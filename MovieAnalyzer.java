...
public class MovieAnalyzer {
    ...
    public static class Movie {
        ...
        public Movie(String Series_Title, int Released_Year, String Certificate,
                     ...
                     String Meta_score, String Director, String[] Stars,
                     int Noofvotes, String Gross) {
    ...
    public static class CoStarCount {
        ...
        public List<String> getStarsName() {
            return starsName;
        }

        public String getFirstStar() {
            return starsName.get(0);
        }

        public String getSecondStar() {
            return starsName.get(1);
        }
    ...
    public MovieAnalyzer(String dataset_path) throws IOException {
        Files.lines(Paths.get(dataset_path))
                .skip(1)
                .map(l -> l.split(",(?=([^\\\"]*\\\"[^\\\"]*\\\")*[^\\\"]*$)", -1))
                .map(a -> new Movie(a[1], Integer.parseInt(a[2]),
                        a[3], Integer.parseInt(a[4].split(" ")[0]), a[5],
                        Float.parseFloat(a[6]), a[7], a[8], a[9],
    ...
    public Map<Integer, Integer> getMovieCountByYear() {
        ...
        Map<Integer, Long> map = moviesStream.collect(
                Collectors.groupingBy(Movie::getReleased_Year, Collectors.counting())
        );
        Set<Integer> set = map.keySet();
        Map<Integer, Integer> ans = new TreeMap<>((o1, o2) -> o2 - o1);
        set.forEach(a -> {
    ...
    public Map<String, Integer> getMovieCountByGenre() {
        ...
        genremap.sort(Comparator.comparing(GenreCount::getCount, Comparator.reverseOrder())
                        .thenComparing(GenreCount::getName)
        );
    ...
    public Map<List<String>, Integer> getCoStarCount() {
        Stream<Movie> moviesStream = movies.stream();
        Map<String, Integer> CoStarmap = new HashMap<>();
        List<CoStarCount> res = new ArrayList<>();
        ...
        for (String str : set) {
            res.add(new CoStarCount(new ArrayList<>(Arrays.asList(str.split(","))),
                    CoStarmap.get(str))
            );
        }
    ...
    public List<String> getTopMovies(int top_k, String by) {
        ...
        if (by.equals("runtime")) {
            moviesStream.sorted(Comparator.comparing(Movie::getRuntime, Comparator.reverseOrder())
                    ...
                    . forEach(a -> ans.add(a.getSeries_Title())
                    );
        } else {
    ...
    public List<String> getTopStars(int top_k, String by) {
        ...
        if (by.equals("rating")) {
            ...
            moviesStream.forEach(a -> {
                if (!starRating.containsKey(a.getStars()[0])) {
                    starRating.put(a.getStars()[0], new ArrayList<Float>());
                }
                if (!starRating.containsKey(a.getStars()[1])) {
                    starRating.put(a.getStars()[1], new ArrayList<Float>());
                }
                if (!starRating.containsKey(a.getStars()[2])) {
                    starRating.put(a.getStars()[2], new ArrayList<Float>());
                }
                if (!starRating.containsKey(a.getStars()[3])) {
            ...
            starmap.sort(Comparator.comparing(Star::getRating, Comparator.reverseOrder())
            ...
            moviesStream.forEach(a -> {
                if (!a.getGross().equals("")) {
                    long aGross = Long.parseLong(a.getGross().replaceAll(",", "")
                                    . replaceAll("\"", ""));
                    if (!starRating.containsKey(a.getStars()[0])) {
                        starRating.put(a.getStars()[0], new ArrayList<>());
                    }
                    if (!starRating.containsKey(a.getStars()[1])) {
                        starRating.put(a.getStars()[1], new ArrayList<>());
                    }
                    if (!starRating.containsKey(a.getStars()[2])) {
                        starRating.put(a.getStars()[2], new ArrayList<>());
                    }
                    if (!starRating.containsKey(a.getStars()[3])) {
                        starRating.put(a.getStars()[3], new ArrayList<>());
                    }
            ...
            starmap.sort(Comparator.comparing(Star::getGross, Comparator.reverseOrder())
                    . thenComparing(Star::getName)
            );
            for (int i = 0; i < top_k; i++) {
                ans.add(starmap.get(i).getName());
